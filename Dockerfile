...
#RUN echo 'create-auth-realm --classname=com.sun.enterprise.security.auth.realm.jdbc.JDBCRealm --property jaas-context=jdbcRealm:datasource-jndi=jdbc/__financialsalmon:user-table=person:user-name-column=username:password-column=pwd:group-table=security_groups:group-table-user-name-column=username:group-name-column=groupname:digest-algorithm=SHA-256:encoding=Base64:charset=UTF-8: financialsalmon_realm' >> $POSTBOOT_COMMANDS